...
Dated snapshot from 2026-08-26. Kept as an example of the shape:
surfaces, contracts, entities, authority, then findings with evidence.

This is not a living issue list. Re-runs belong in a scratch folder
(`*.detangler-app/`), not here.

**After this pass**

- **F-001–F-005** fixed in `867f850` (pocket card paths, AppFacts png
  line, SkillFacts version and bundle list, `core.md` zip inventory).
- **F-006** left. `cursor-skill` is the SkillFacts kind, not the install
  story.
- **F-007** left. The Files table is the index. The raw-link line is
  incomplete, not broken.
- **F-008 / F-009** left on purpose. Short `/install` and `/skill` pages
  that restate dest dirs are the house skill-page shape. Docs is the
  long copy. Do not collapse the short pages into pointers.
