# Spec: Fold the talk-register harvest into Smell Check

*Opened 2026-09-02. Source: the What Over How overlay
(`z:\workspace\what-over-how\docs\smellcheck.md`, section
"Unveiling talk"), ruled 2026-09-01 through 2026-09-02 while
rewriting a vision talk. Ask: some of those rules travel. Write
...
## 1. What landed in the overlay

The overlay grew a talk register so an agent drafting as Sam
would stay beside the listener. Most of the bullets were named
from a failed sentence in that talk, then generalized one step.

A few of those bullets are house dialect: Wonder Talk, the
quest, locked space/time, "the work" as not his. Those stay in
the consuming overlay.

The rest are sentence tells an agent will hit in any repo.
Those belong here. Overlay may tighten core. It does not loosen
it. House examples stay house. Package examples stay generic.
...
2. **Prefer cousins of existing tells.** If a harvest bullet is
   the same waste as setup, riddle label, unparseable sentence,
   or parallel-zinger, extend that heading. Do not mint a
   fifteenth cadence name.
3. **Pocket card stays a digest.** At most three new lines, and
   only for core tells agents hit without opening `essays.md`.
   Genre law stays out of `cursor.mdc`.
4. **Generic bad → good in the package.** Provenance table
   below keeps the house sentence that taught the rule. Do not
   ship Living Legacy, the attic, or the nineties book as the
   package example.
5. **Carve-outs where a genre already earns the costume.**
   Academic "the work" meaning this study. Civic ceremony that
   names a real oath. Applicability heuristic still wins.

---

## 3. The sort

| Overlay name | Destination | Move |
| --- | --- | --- |
| Don't announce the receipt | `core.md` setup sentences | Extend. Already banned. Add one example. |
| No clever pairings that need decoding | `core.md` parallel-zinger | Extend. |
...
| Interior vs exterior | overlay | Stay. |
| Don't turn a quest into an assignment | overlay | Stay. |
| Driver before admin frame | overlay | Stay. |
| Understanding, not a talk given / contextual translator | overlay | Stay. |
| Locked pairs (space / time / asymmetry) | overlay | Stay. |
| No em dashes in publishable prose | overlay | Stay. Core allows an earned dash. |

---

## 4. Already covered (do not add a second ban)

**Don't announce the receipt** is setup sentences. Core already
bans a sentence whose only job is to promise the next one.
Cataphoric teasers are the dramatic cousin. Add the quiet
cousin to Bad → good:

- Bad: "That project started earlier than the repo, and I can
  date it."
- Good: "I started it in 2019."

**Socratic beats stay.** Core already says real questions aimed
at a reader are method, not "transition question" smell. The
new essays rule is the other half: a theory question before
the picture is not a Socratic beat. It is a unit title.
...
Under **parallel-zinger**, add: a pairing the reader has to
decode is the same spice overspent. "Most lists grow out of
opportunities. This one grew out of conclusions." Spell the
fact. Trim assistant-added pairings first. Native pairs still
stand.

Under **riddle labels**, add two smaller units of the same
withhold:

- A dummy scale label ("On a single life,") after the scale
  is already in the room. If the shift is real, do it with
  *so* or a verb. If it is not, cut it.
- A compression label ("The through-line, compressed, is five
...
Do not ask a theory question before the picture is in the
room. "Why does recognition work where instruction fails?" is
a unit title in question form. Ask after they can see the
thing, or do not ask: start with the strike, the tree, the
hinge.

A Socratic question starts with something ordinary. A midterm
question starts with the doctrine. Core already protects the
first. This rule names the second.

### 6.3 Take a breath

A new paragraph is a pause. If you would stop and look at
them before the next beat, break. Do not chop every sentence.
...
## 7. Stay in the consuming overlay

Leave these where they are. They fail the "any repo" test.

- **Unveiling talk** as a named register: beside the listener,
  not above them. That is a project voice, like civic ceremony.
- **I / we / you** for Sam introducing a vision or a Wonder
  Talk. Core already says pronouns are a project decision.
- **Wonder Talk questions stay.** Method, not lecture. Point
  at core's Socratic protection; do not restate a house
  transmission method as package law.
- **Interior vs exterior.** If he could see it, say so. That
  is his narrative of seeing.
- **Quest, not assignment. Driver before admin frame.**
  Wonder and curiosity first. House vocation.
- **Understanding achieved, not a talk given.** Contextual
  translator. Primer-path doctrine.
- **Locked pairs.** Space / time / unlock, dynamo / ratchet /
  asymmetry. Named set in that corpus. The general cousin
  (do not ship one half of a defined pair; do not freeze a
  count that has grown) is tempting. It is also easy to
  over-apply. Leave it until a second consumer needs it.
- **No em dashes** in publishable What Over How prose. Core
  already allows an earned dash. Overlay stays stricter.

After this package lands, the vault overlay can slim: keep
the unveiling register, the pronoun policy, the term table,
protected testimony, and a pointer at the new core / essays
tells. That slim is a vault pass. Not this one.

---

## 8. Files this rewrite would touch

Canon is `rules/`. `scripts/sync-skill-static.mjs` copies onto
the skill folder, the site, and `.cursor/skills/smellcheck/`.
Edit canon. Sync on `pnpm test` / site build. Do not hand-edit
the copies.
...
## 10. Provenance (house sentences that taught the rule)

For the rewrite, not for the package examples.
...
| Obligation as object | "I take the obligation" |
| Receipt | "That quest started earlier than the pages, and I can date it." |
| Compression label | "The through-line, compressed, is five moves" |
| Dummy scale | "On a single life," |
| Midterm question | "Why does recognition work where instruction fails?" |
| Pairing | "Most project lists grow out of opportunities. This one grew out of conclusions." |
| Impersonal | "Show one piece and it looks like a side project." |
| Live definition | "Conclusions" for a still-refining shape |
| Breath | The arc's *then* chain: person, then motion-and-joining, then holding-and-locking |
| Repeat / stall | Second pass over the nineties book that only restated it |
